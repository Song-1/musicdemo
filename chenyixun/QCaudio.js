...
MPlayer = {
		...
		play : function(data){
			...
			$('.playback').onclick=function(){
				...
				$('.playback').className='playback icoon';
			
				MPlayer._player.pause();

			}
		
		if(MPlayer._player.duration!=""){
			
			function objtimer(){
				
		    var currentSec=parseInt(MPlayer._player.currentTime%60)<10 ?'0'+parseInt(MPlayer._player.currentTime%60): parseInt(MPlayer._player.currentTime%60);
			
			var ratio=MPlayer._player.currentTime/MPlayer._player.duration*100;

		    document.getElementById("pace").style.width=ratio+"%";
			
			document.getElementById("Musictime").innerHTML=parseInt(MPlayer._player.currentTime/60)+':'+currentSec;
		...
		pause : function(){
			...
			$('.playback').className='playback icoon playing';
			...
			if( true ){
				...
				var MP= MPlayer;
				  ...
				  document.getElementById("player").innerHTML=
			       ...
			       '<div class="control">'+
				   '<div class="left">'+
				   '<div class="rewind icoon" style="dislpay:none; visibility:hidden; height:0px; width:0px;"></div>'+
					'<div class="playback icoon"></div>'+
					'<div class="fastforward icoon"style="dislpay:none; visibility:hidden; height:0px; width:0px;"> </div>'+
				'</div>'+
